...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500">Unggah foto dan rincian produk untuk ditampilkan di etalase store website.</p>
        </div>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="tagline" value="{{ old('tagline') }}" placeholder="Contoh: Platform Digitalisasi Desa Terpadu" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#fe6000] focus:outline-none" />
            </div>
            ...
            <div class="space-y-1.5 md:col-span-2">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200">
                    <img id="product_thumb_preview" src="/btd/0.png" alt="Preview Thumbnail" class="w-28 h-20 rounded-xl object-cover border-2 border-white shadow-sm shrink-0 bg-slate-200" />
                    <div class="space-y-2 flex-1">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="thumbnail" value="{{ old('thumbnail', '/btd/0.png') }}" placeholder="/images/products/..." class="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-[11px] mono text-slate-600 focus:outline-none" />
                        </div>
                    </div>
                </div>
            </div>
