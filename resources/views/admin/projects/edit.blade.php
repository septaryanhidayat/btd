...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500">Perbarui informasi portofolio {{ $project->title }}</p>
        </div>
    ...
    <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="thumbnail" value="{{ old('thumbnail', $project->thumbnail) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none" />
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select name="app_type" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none">
                    <option value="web" {{ old('app_type', $project->app_type) == 'web' ? 'selected' : '' }}>🖥️ Web Desktop / SaaS (Ukuran Landscape 16:9)</option>
                    <option value="mobile" {{ old('app_type', $project->app_type) == 'mobile' ? 'selected' : '' }}>📱 Mobile App (Ukuran Smartphone Portrait 9:16)</option>
                    <option value="both" {{ old('app_type', $project->app_type) == 'both' ? 'selected' : '' }}>💻📱 Hybrid / Keduanya (Responsive)</option>
                </select>
            </div>
            ...
            <div class="space-y-1.5 md:col-span-2">
                ...
                <textarea name="gallery_raw" rows="4" placeholder="Format per baris: Path Gambar | Judul Layar | Tipe (web/mobile) | Keterangan&#10;Contoh:&#10;/images/products/smart-village-mockup.jpg | Dashboard Pelayanan | web | Antarmuka desktop staf desa&#10;/images/surat.png | Format Surat Resmi | web | Otomasi cetak dokumen ber-QR code" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs mono focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none">{{ old('gallery_raw', $galleryText) }}</textarea>
