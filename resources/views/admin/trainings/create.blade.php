...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500">Unggah foto dan rincian silabus untuk program workshop / seminar.</p>
        </div>
    ...
    <form action="{{ route('admin.trainings.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="level" required value="{{ old('level', 'Executive & Dosen') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none" />
            </div>

            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-[#07153f]">Estimasi Durasi Pelatihan</label>
                <input type="text" name="duration" value="{{ old('duration', '1 Hari Workshop Intensif') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none" />
            </div>

            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-[#07153f]">Biaya Investasi / Harga (Rp)</label>
                <input type="number" name="price" value="{{ old('price', 1500000) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs mono font-bold focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none" />
            </div>

            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-[#07153f]">Target Peserta</label>
                <input type="text" name="target_audience" value="{{ old('target_audience', 'Dosen, Akademisi & Pengajar') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none" />
            </div>
            ...
            <div class="space-y-1.5 md:col-span-2">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200">
                    <img id="training_thumb_preview" src="/images/Insight-Talks-Komdigi.jpeg" alt="Preview" class="w-28 h-20 rounded-xl object-cover border-2 border-white shadow-sm shrink-0 bg-slate-200" />
                    <div class="space-y-2 flex-1">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="thumbnail" value="{{ old('thumbnail', '/images/Insight-Talks-Komdigi.jpeg') }}" class="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-[11px] mono text-slate-600 focus:outline-none" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-1.5 md:col-span-2">
                ...
                <textarea name="syllabus_raw" rows="4" placeholder="Konsep Arsitektur RAG & Vector Database&#10;Implementasi Prompt Engineering&#10;Deployment Model AI" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-[#3E5CE7] focus:outline-none">{{ old('syllabus_raw') }}</textarea>
            </div>
