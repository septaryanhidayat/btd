...
<section class="py-20 bg-white transition-colors duration-300 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        ...
        <div class="text-center space-y-3">
            <h1 class="text-4xl sm:text-5xl font-extrabold text-[#07153f] dark:text-white">Portofolio</h1>
            <p class="text-base text-slate-600 dark:text-slate-300 max-w-2xl mx-auto font-medium">
        ...
        <div class="flex flex-wrap items-center justify-center gap-2">
            <a href="{{ route('projects.index') }}" 
               class="px-5 py-2.5 rounded-xl text-xs font-bold transition-all {{ !request('category') ? 'bg-[#3E5CE7] text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700' }}">
                Semua Proyek
            </a>
            @foreach($categories as $cat)
                <a href="{{ route('projects.index', ['category' => $cat->slug]) }}" 
                   class="px-5 py-2.5 rounded-xl text-xs font-bold transition-all {{ request('category') === $cat->slug ? 'bg-[#3E5CE7] text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($projects as $project)
                <div class="bg-white dark:bg-slate-800/90 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col group">
                    <div class="relative aspect-video overflow-hidden bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <div class="absolute top-3 left-3">
                            <span class="px-3 py-1 rounded-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-md text-[#3E5CE7] dark:text-blue-400 font-bold text-[10px] shadow-xs">
                    ...
                    <div class="p-6 flex-grow flex flex-col justify-between space-y-4">
                        <div class="space-y-2">
                            ...
                            <h3 class="text-lg font-extrabold text-[#07153f] dark:text-white group-hover:text-[#3E5CE7] transition-colors line-clamp-1">
                                {{ $project->title }}
                            </h3>
                        ...
                        <div class="pt-3 border-t border-slate-100 dark:border-slate-700">
                            <a href="{{ route('projects.show', $project->slug) }}" class="w-full py-3 rounded-xl bg-[#2A334B] dark:bg-slate-700 hover:bg-[#3E5CE7] text-white font-bold text-xs uppercase text-center block shadow-xs transition-all">
            ...
            @empty
                <div class="col-span-3 text-center py-12 text-slate-500 dark:text-slate-400 font-medium">
...
<section class="py-16 bg-[#f8faff] dark:bg-slate-950 overflow-hidden border-t border-slate-200 dark:border-slate-800 marquee-pause relative">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 text-center relative z-10">
        <h2 class="text-3xl font-extrabold text-[#07153f] dark:text-white">Client</h2>
