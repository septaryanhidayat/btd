...
<section class="py-12 lg:py-16 bg-white dark:bg-[#070A11]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div class="text-center max-w-3xl mx-auto space-y-4">
            ...
            <h1 class="text-3xl sm:text-5xl font-extrabold text-slate-950 dark:text-white font-heading">Insights & Tech Blog</h1>
        ...
        <div class="max-w-md mx-auto">
            <form action="{{ route('blog.index') }}" method="GET" class="flex gap-2">
                <input type="text" 
                       ...
                       class="flex-1 px-4 py-3 rounded-2xl bg-white dark:bg-slate-900 text-sm font-bold text-slate-900 dark:text-white border border-slate-300 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-xs" />
                <button type="submit" class="px-6 py-3 rounded-2xl bg-indigo-700 hover:bg-indigo-800 text-white font-extrabold text-sm shadow-md transition-all">
                    Cari
                </button>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse($posts as $post)
                <article class="bento-card overflow-hidden flex flex-col group border border-slate-200 dark:border-slate-800">
                    ...
                    <div class="p-6 flex-grow flex flex-col justify-between space-y-4">
                        <div class="space-y-2">
                            <span class="text-[11px] text-indigo-700 dark:text-indigo-400 font-extrabold">
                                {{ $post->published_at ? $post->published_at->format('d M Y') : $post->created_at->format('d M Y') }}
                            </span>
                            <h2 class="text-lg font-extrabold text-slate-950 dark:text-white group-hover:text-indigo-700 dark:group-hover:text-indigo-400 transition-colors line-clamp-2">
                                {{ $post->title }}
                            </h2>
                        ...
                        <div class="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                            <span class="font-extrabold text-slate-700 dark:text-slate-400">Oleh {{ $post->author?->name ?? 'Septa Ryan Hidayat' }}</span>
                            <a href="{{ route('blog.show', $post->slug) }}" class="font-extrabold text-indigo-700 dark:text-indigo-400 hover:underline">
                                Baca Artikel &rarr;
                            </a>
                        </div>
                    </div>
